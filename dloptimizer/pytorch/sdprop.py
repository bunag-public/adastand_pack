...
class SDprop(torch.optim.Optimizer):
    """Implements SDprop algorithm.
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('SDprop does not support sparse gradients')
                state = self.state[p]
                ...
                square_avg.mul_(alpha).addcmul_(alpha * (1 - alpha), grad- grad_avg, grad - grad_avg)

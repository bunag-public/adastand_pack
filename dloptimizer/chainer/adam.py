...
_default_hyperparam.adastand = False
...
class AdamRule(optimizer.UpdateRule):
    ...
    Args:
        ...
        adastand (bool): Whether to use the Adastand variant of Adam.

    """

    def __init__(self, parent_hyperparam=None,
                 alpha=None, beta1=None, beta2=None, eps=None,
                 eta=None, weight_decay_rate=None, amsgrad=None, adastand=None):
    ...
    def update_core_cpu(self, param):
        ...
        if hp.adastand:
            v += (1 - hp.beta2) * (hp.beta2 * (grad - m)*(grad - m) - v)
            m += (1 - hp.beta1) * (grad - 2 * m)
        else:
            m += (1 - hp.beta1) * (grad - m)
    ...
    def update_core_gpu(self, param):
        ...
        if hp.eps != 0 and eps == 0:
            raise ValueError(
                'eps of Adam optimizer is too small for {} ({})'.format(
                    grad.dtype.name, hp.eps))
        if hp.amsgrad and hp.adastand:
            cuda.elementwise(
                'T grad, T lr, T one_minus_beta1, T one_minus_beta2, T eps, \
                 T eta, T weight_decay_rate, T beta2',
                'T param, T m, T v, T vhat',
                '''v += one_minus_beta2 * (beta2 * (grad - m)*(grad - m) - v);
                   m += one_minus_beta1 * (grad - 2 * m);
                   vhat = max(vhat, v);
                   param -= eta * (lr * m / (sqrt(vhat) + eps) +
                                   weight_decay_rate * param);''',
                'adam')(grad, self.lr, 1 - hp.beta1,
                        1 - hp.beta2, hp.eps,
                        hp.eta, hp.weight_decay_rate, hp.beta2,
                        param.data, self.state['m'], self.state['v'],
                        self.state['vhat'])
        elif hp.amsgrad and not hp.adastand:
            cuda.elementwise(
                'T grad, T lr, T one_minus_beta1, T one_minus_beta2, T eps, \
                ...
                'adam')(grad, self.lr, 1 - hp.beta1,
                        ...
                        param.data, self.state['m'], self.state['v'],
                        self.state['vhat'])
        elif not hp.amsgrad and hp.adastand:
            cuda.elementwise(
                'T grad, T lr, T one_minus_beta1, T one_minus_beta2, T eps, \
                 T eta, T weight_decay_rate, T beta2',
                'T param, T m, T v',
